...
main:
        ...
        mov     x19, x1                      //store the command line arguments in a temp register
        ...
        ldr     x0, [x19, 8]                 //load the 1st input passed onto x0
        mov     x26, x0                //move the integer to x20

        //storing 2nd argument
        ldr     x0, [x19, 16]                //load the 2nd input passed onto x0
        bl      atoi                            //convert the argument to integer
        mov     x20, x0                      //move the integer to x21

        //validation for 2nd argument
        cmp     x20, 10                       //compare it with 5
        b.lt    wrongInput2                     //if less, branch to wrongInput3
        cmp     x20, 200                      //compare it with 20
        b.gt    wrongInput2                     //if more, branch to wrongInput3

        //storing 3rd argument
        ldr     x0, [x19, 24]                //load the 2nd input passed onto x0
        bl      atoi                            //convert the argument to integer
        mov     x21, x0                      //move the integer to x21

        //validation for 3rd argument
        cmp     x21, 10                       //compare it with 5
        b.lt    wrongInput3                     //if less, branch to wrongInput3
        cmp     x21, 200                      //compare it with 20
	...
	mov	x1, x26		//pass playerName as argument
	...
	mov	x19, xzr			//initialize temp
	mov	x27, xzr		//initialize positive percentage
...
loop1:
	...
	add	x19, x19, 1		//incrementing temp
...
next2:
	...
	add	x27, x27, 1	//increment positive percentage

next3:
        add     x23, x23, 1                     //increment j
        b       cmp2                            //branch to cmp2

cmp1:
        cmp     x23, x21                     //compare j to cols
        b.lt    loop1                           //if less, branch to loop1

        add     x22, x22, 1                     //increment i
        mov     x23, xzr                        // j = 0
        ldr     x0, =output5                    //load print message "\n"
        bl      printf                          //print "\n"

cmp2:
        cmp     x22, x20                     //compare i to rows
        b.lt    cmp1                            //if less, branch to cmp1

	sub	x27, x27, x19	//posPercent= posPercent - temp

//Negative Num Percentage
percentage:
	mov	x0, 100				//x0 = 100
	scvtf	d0, x19			//d0 = temp
	scvtf	d1, x20			//d1 = rows
	scvtf	d2, x21			//d2 = cols
...
//Special Tiles Percentage
	mov	x0, 100				//x0 = 100
	add	x1, x27, x19	//x1 = posPercent + temp
...
board:
	mov	x22, 0				//move 0 to i
	mov	x23, 0				//move 0 to j
